...
interface Props {
  currentPower: number;
  currentTotalAccumulated: number;
  netDay: number;
  netDayHytta: number;
...
class TallPanelDisplay extends React.PureComponent<Props, GenericProps> {
  ...
  private getMinUsage(hytta: boolean) {
    ...
    if (prodMax > 0) {
      ...
      return (
        <TellulfInfoCell key="maxProd" info={useMin} header="min forbruk" smartRoundKw unit="W" />
      );
    }
  }
  ...
  public render(): React.ReactNode {
    return (
      ...
      >
        <div className="energyTableRow energyTableRowHeader">Totalt</div>
        <div className="energyTableRow">
          <TellulfInfoCell
            info={this.props.currentNetConsumption}
            unit="W"
            header="nå"
            decimals={1}
            smartRoundKw
